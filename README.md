...
## September 19 sixteenth pass: R10 frontier (first rank-1 turnover), C17–C19 models, exec label fix, verifier crash fix

- **Tenth official frontier capture (R10, N=99,663).** `HappyLittleTrades` takes rank 1 at **+994.30% / $2,485,745.50** (promoted from #3 — the first rank-1 turnover of the edition, H40 supported); `youcanttrickme` falls to #4 at +908.01% / $2,270,023.00 (H20). Ranks 1–250 fully re-enumerated into `research/evidence/TV-CONTEST-AMP-SEP2026-R10.md`; the live returns row re-synced (994.30% / 10.943x / N=99,663).
- **Rules R10 re-verified line by line: 94/94 symbols + caps + order match, 0 equities.** One text change found: the payment rail moved from "≥$1000 wire-or-PayPal" to "≥$600 wire-only" (IR-29, low, flagged for review; payout config stays frozen at the $1000 reading). Landing hero (99,658) trails the contest count (99,663) by the usual async lag (IR-09); all 15 completed champions re-read verbatim with 0 mismatch (max 53.2072x — no 100x in verified history).
- **Three new frozen stock models + six usernames (roster 32→38, daily 28→34).** C17 overnight implosion harvester (gap-down + absorbed-panic long), C18 blow-off short avalanche (short vertical runs, pyramid into extension — the deliberate tail bet), C19 twin-hammer capitulation compounder (two-day liquidation cascade long). Hypotheses H37–H39, verdict withheld: on the partial 11-symbol daily pool C19 never fired ($0 season), C17 lost small, C18 lost ~$10–12k — first-fire and full-pool verdicts pending.
- **Executive summary now speaks plainly.** The 3 pending orders render as sentences ("Close the short position and open a LONG position (~1 unit) in `CME:ETH1!` …"), including the first-ever daily ADD (`BreakoutBea` +~50 MSTR); a builder bug that labelled that ADD "new position" is fixed ("increase the open position"), and a header capsule links the very top of the page to the order list.
- **Two verifier fixes.** `returns.live_consistency` now uses the project's own H11 rounding bound ($12.50, not $1.00) so a verbatim official %/$ pair with $4.50 of rounding slack passes; a latent crash (missing capture file → `FileNotFoundError` once the symbol lands in the study) is fixed by cross-checking only loaded captures and raising `IntradayError` on unreadable files.
- **Pass 2 bugfix sweep (same session).** Full-diff re-read of every hand-written change + independent re-derivation of the R10 figures (94/94 symbols+caps+order re-matched, frontier pairs and H40 R9-comparisons re-checked against committed evidence, #250 exactly at the $12.50 rounding bound). Fixes: C17 claim text now states the green-bar requirement the frozen code enforces (`MODEL_CLAIMS` + H37; artifacts + site regenerated); new `.compact` CSS rule for the order lists; `docs/assets/` mirrored so the docs page renders styled (it linked a missing stylesheet); H34–H36 evidence addenda superseding the stale ENPH-only note (11/20 daily captured); README audit banner updated to the 37/60 partial-pool reality. Both pages + stylesheets served and fetched over HTTP (all 200, no dead anchors, no template leakage).
- Verification: **401 passed / 0 failed / 1 warning (IR-22)**, self-test **459/0**, **94 unit tests green**. Details: [the three-pass review](research/implementation_review.md).
