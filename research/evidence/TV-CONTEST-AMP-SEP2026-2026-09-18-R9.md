# TradingView official contest capture — AMP Futures September 2026, pass 9

- **URL:** https://www.tradingview.com/the-leap/amp-futures-september-2026/
- **Publisher:** TradingView, Inc.
- **Accessed (UTC):** 2026-09-18, approximately 17:00 UTC
- **Tier:** official_primary (TradingView, Inc., contest organiser)
- **Use:** point-in-time public leaderboard frontier and displayed participant count only
- **Status:** official primary capture; not a final cutoff and not a prize guarantee

The page displayed **96,707 participants** and exposed the top 250. The following rows were read
verbatim from the official page at this pass:

> 1 — leonardo22_romano — +934.05% — +$2,335,125.00
>
> 2 — youcanttrickme — +929.89% — +$2,324,723.00
>
> 3 — HappyLittleTrades — +826.37% — +$2,065,935.50
>
> 4 — huliusalecsander — +786.78% — +$1,966,940.00
>
> 5 — pratik_j — +775.94% — +$1,939,840.00
>
> 6 — accurateDogabdd0 — +763.34% — +$1,908,353.14
>
> 7 — S_P_I_D_E_Y — +753.29% — +$1,883,230.00
>
> 8 — leongoat — +743.75% — +$1,859,365.00
>
> 9 — gautamkumarpaswn — +729.93% — +$1,824,835.00
>
> 10 — Danaos — +728.13% — +$1,820,337.50

### Landing page champions (verified unchanged)

All 15 completed-edition champion records from the official landing page were verified:
- Maximum completed outcome: **53.2072×** (BenBernanke1, CME edition)
- No completed edition at or above 100×
- New champions visible: Me-tis (TradeStation Jul 2026, +155.95%), Minitrader91 (Forex Jun 2026, +91.87%), Manishh_jain92 (Crypto May 2026, +271.78%), prodigy5284 (Stocks Mar 2026, +17.58%)

### Key observations

1. Participants rose from 96,586 (R8) to 96,707 (+121).
2. Rank 1 unchanged at +934.05% / $2,335,125.00 (leonardo22_romano).
3. Rank 2 is now youcanttrickme (+929.89%), very close to rank 1.
4. The leaderboard is very competitive at the top with ranks 1-10 all above 700%.
5. Registration still open until Sep 23, 2026.

### No hallucination check

- All rank, trader, percentage, and dollar values are copied verbatim from the fetched page.
- Participant count 96,707 matches the "This is the top 250 out of 96,707 participants" display.
- Champion summaries match the landing page section exactly.
- URL for rules: https://www.tradingview.com/the-leap/amp-futures-september-2026/rules/
- URL for contest: https://www.tradingview.com/the-leap/amp-futures-september-2026/
- URL for landing: https://www.tradingview.com/the-leap/
